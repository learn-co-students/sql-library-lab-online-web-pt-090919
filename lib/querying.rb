...
def select_series_title_with_most_human_characters
  ...
  LEFT OUTER JOIN characters ON series.author_id = characters.author_id WHERE species = 'human' GROUP BY series.title ORDER BY COUNT(species) DESC LIMIT 1"
end
...
def select_character_names_and_number_of_books_they_are_in
  ...
  JOIN books ON character_books.book_id = books.id GROUP BY characters.name ORDER BY COUNT(characters.name) DESC"
end
